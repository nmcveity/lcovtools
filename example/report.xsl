...
<HTML>
<HEAD>
<TITLE>Coverage: <xsl:value-of select="floor((/CoverageReport/Summary/@totalVisited div /CoverageReport/Summary/@totalValid) * 100)"/>%</TITLE>
<STYLE type="text/css">
@import url("lua.css");
...
<BODY>
    ...
    <BLOCKQUOTE>
    <TABLE>
        <TR><TD><B>Source coverage:</B></TD><TD><xsl:value-of select="floor((/CoverageReport/Summary/@totalVisited div /CoverageReport/Summary/@totalValid) * 100)"/>%</TD></TR>
	...
	<TABLE class="contents">
		<xsl:for-each select="/CoverageReport/File">
            <xsl:sort select="Summary/@coverage" data-type="number" order="descending"/>
            <TR>
            <TD class="contents">
                <A>
...
<xsl:template name="CoverageIndicator">
    <xsl:variable name="coverage"><xsl:value-of select="./Summary/@coverage"/></xsl:variable>
...
<xsl:template match="File">
    <H2><A><xsl:attribute name="name"><xsl:value-of select="@name"/></xsl:attribute>File <xsl:value-of select="@name"/></A></H2>
